...
try {
    ...
    $twoFactorService = new TwoFactorService($pdo);

    $authService = new AuthService($userModel, $tokenService, $validationService);
    $videoService = new VideoService($videoModel, $commentModel, $reactionModel);
    $reactionService = new ReactionService($reactionModel, $videoModel);
    $commentService = new CommentaireService($commentModel, $videoModel);
    $userService = new UserService($userModel, $videoModel, $videoViewModel);
    $analyticsService = new AnalyticsService($pdo);
    $notificationService = new NotificationService($notificationModel);

    AuthMiddleware::init($tokenService, $userModel);

    $authController = new AuthController($authService, $twoFactorService);
    $videoController = new VideoController($videoService, $fileUploadService, $analyticsService);
    $reactionController = new ReactionController($reactionService);
    $commentController = new CommentaireController($commentService);
